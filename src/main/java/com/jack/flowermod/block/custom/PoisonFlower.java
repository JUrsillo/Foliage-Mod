...
public class PoisonFlower extends FlowerBlock {
    ...
    public PoisonFlower(MobEffect pSuspiciousStewEffect, int pEffectDuration, Properties pProperties) {
    ...
    public void animateTick(BlockState pState, Level pLevel, BlockPos pPos, Random pRandom) {
        ...
        for(int i = 0; i < 6; ++i) {
            if (pRandom.nextBoolean()) {
                pLevel.addParticle(ParticleTypes.WHITE_ASH, d0 + pRandom.nextDouble() / 5.0D, (double)pPos.getY() + (0.5D - pRandom.nextDouble()), d1 + pRandom.nextDouble() / 5.0D, 0.0D, 0.0D, 0.0D);
            }
        }
        ...
        if (!pLevel.isClientSide && pLevel.getDifficulty() != Difficulty.PEACEFUL) {
            if (pEntity instanceof LivingEntity) {
                ...
                if (!livingentity.isInvulnerableTo(DamageSource.MAGIC)); {
                    livingentity.addEffect(new MobEffectInstance(MobEffects.POISON, 120));
                }
            }
